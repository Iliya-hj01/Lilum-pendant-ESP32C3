...
static constexpr OrchestraRole ORCHESTRA_ROLE = OrchestraRole::Follower;
static constexpr int16_t ORCHESTRA_POSITION = -1;  // Undefined until position mapping is finalized.
static constexpr uint16_t ORCHESTRA_MASTER_ID = 1;

// BLE timing plan (milliseconds).
// Master: advertise 60 s on startup, then 2 s on / 38 s off.
...
static constexpr uint32_t ORCHESTRA_CYCLE_OFF_MS = 38000;
static constexpr uint32_t ORCHESTRA_SYNC_GUARD_MS = 120;

// BLE advertising payload metadata.
static constexpr uint16_t ORCHESTRA_ADV_COMPANY_ID = 0xFFFF;
static constexpr uint8_t ORCHESTRA_ADV_DATA_VERSION = 3;
